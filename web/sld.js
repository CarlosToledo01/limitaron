(function(){
  const COLORS = {
    ...
    // Colores para conductores multi-fase (bifásico/trifásico)
    multiBifa: '#0ea5e9',
    multiTrifa: '#9333ea'
  };
  ...
  function buildCircuits(s){
    ...
    (s.derived_proposal||[]).forEach(ci=>{
      ...
      byId.set(id,{
        ...
        tipo_conexion: ci.tipo_conexion, valido: ci.valido
      });
    });

    const circuits=[];
    const all = s.derived_proposal || [];
    ...
    if(s.balanceEnabled && s.phase_balance?.assignment?.length){
      ...
      // También mostrar circuitos no balanceados (bifa/trifa)
      all.forEach(ci=>{
        const id = `${ci.key}_${ci.circuito_index}`;
        if(!assignedIds.has(id)){
          const base=byId.get(id)||ci;
          circuits.push({...base, fase:1, id});
        }
      });
    } else {
      byId.forEach(c=> circuits.push({...c,fase:1}));
    }
  ...
  function generateSLD(s, opts={}){
    ...
    // Barra principal (sin "sombras" de fase)
    ...
    function tuboForType(tipo){
      ...
      if(tipo==='Contacto específico') return cpm.contactos_especificos?.tubo;
      return undefined;
    }
    ...
    circuits.forEach((ci,i)=>{
      ...
      // Color del conductor: por fase o multi-fase
      const conductorColor =
        ci.tipo_conexion==='bifa' ? COLORS.multiBifa :
        ci.tipo_conexion==='trifa' ? COLORS.multiTrifa :
        (COLORS.phaseBar[ci.fase] || '#000');
      ...
      else if(ci.tipo==='Contacto específico') loadSymbol = symbolSpecific(x, loadY);
      ...
      // Mover etiquetas a la derecha del conductor para evitar empalmes
      const labelX = x + 18;
      ...
      // Texto de fases (lateral) y etiqueta sobre el bus
      const phaseText =
        ci.tipo_conexion==='bifa' ? 'Fases F1F2' :
        ci.tipo_conexion==='trifa' ? 'Fases F1F2F3' :
        `Fase F${T(ci.fase)}`;

      const topPhaseText =
        ci.tipo_conexion==='bifa' ? 'F1F2' :
        ci.tipo_conexion==='trifa' ? 'F1F2F3' :
        `F${T(ci.fase)}`;

      body += `
        <g>
          <!-- Etiqueta sobre el bus, centrada en cada derivado, con color del conductor -->
          <text x="${x}" y="${busY-8}" font-size="13" text-anchor="middle" font-weight="700"
                fill="${conductorColor}">${topPhaseText}</text>
          ...
          <text x="${labelX}" y="${breakerY-40}" font-size="15" text-anchor="start" fill="${COLORS.breakerBranch}" font-weight="700">
            ${T(ci.breaker)}A
          </text>
          <text x="${labelX}" y="${busY-26}" font-size="15" text-anchor="start" fill="${COLORS.text}" font-weight="700">
            C${T(ci.idx)}
          </text>
          <text x="${labelX}" y="${t1}" font-size="12" text-anchor="start" fill="${COLORS.text}">
            ${Math.round(ci.VA||0)} VA
          </text>
          <text x="${labelX}" y="${t2}" font-size="12" text-anchor="start" fill="${COLORS.text}">
            ${T(ci.cal)} TW
          </text>
          <text x="${labelX}" y="${t3}" font-size="12" text-anchor="start" fill="${COLORS.text}">
            ${phaseText}
          </text>
          <text x="${labelX}" y="${t4}" font-size="12" text-anchor="start" fill="${vdColor}">
